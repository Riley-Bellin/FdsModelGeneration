document.addEventListener('DOMContentLoaded', function () {
  ...
  /* Cube Animation: Move and Shrink the Cube to the Top Right with a Stylistic Transition */
  const cube = document.getElementById('cube');
  // After 3 seconds, capture the cube's current position and animate it to the top right of the viewport.
  setTimeout(() => {
    // Get the cube's current bounding rectangle
    const rect = cube.getBoundingClientRect();
    // Set cube's position to fixed so that we can animate it relative to the viewport.
    cube.style.position = 'fixed';
    cube.style.top = rect.top + 'px';
    cube.style.left = rect.left + 'px';
    cube.style.margin = '0'; // remove any auto margins
    // Force reflow to ensure the initial position is applied
    void cube.offsetWidth;
    // Apply transition styles (the CSS already has a transition rule on .cube)
    // Animate to target position (for example: 20px from the top and 20px from the right)
    cube.style.top = '20px';
    cube.style.left = 'calc(100vw - 120px)'; // adjust as needed (120px approximates final width including borders)
    // Shrink the cube via scaling
    cube.style.transform = 'scale(0.3)';
    // Stop the continuous rotation animation
